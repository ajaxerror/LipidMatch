...
AppendFrag <- function (CommentColumn, RowStartForFeatureTableData, InputDirectory, NegPos){
  ...
  for (i in 1:length(All_Info_Files)){
    ...
    if (length(Frags)>0){
      ...
      if (NegPos=="Neg") {
        ...
        FragsFilled[(RowFragsFilled+1):nrow(FragsFilled),4]<-gsub("_Pos.*","",All_Info_Files[i])
      }
    }
  ...
  if (NegPos=="Neg") {
    ...
    NegIDed <- read.csv(paste(InputDirectory,"Output/PosIDed.csv",sep=""), header=FALSE)
  }
  ...
  if (NegPos=="Neg") {
    ...
    write.table(NegIDed, paste(InputDirectory,"Output/PosIDed_Fragments.csv",sep=""), sep=",",col.names=FALSE, row.names=FALSE, quote=TRUE, na="NA")
  }
}

...
remove_duplicates<-function(data,row_start,adduct_col,annotation_col,window){
  ...
  for(i in unique(top_ranked_lipid)){
    ...
    if(length(rows)==1){
      ...
      if(grepl("+", paste(all_adducts,sep = "",collapse = "|")) && ("[M-H]-" %in% all_adducts || "[M-2H]-" %in% all_adducts || "[M+C2H3O2]-" %in% all_adducts || "[M+HCO2]-" %in% all_adducts || "[M+HCOO]-" %in% all_adducts || "[M+Cl]-" %in% all_adducts)){
        negative_adduct_indices<-c(match("[M-H]-",all_adducts),match("[M-2H]-",all_adducts),match("[M-2H]-",all_adducts),match("[M+C2H3O2]-",all_adducts),match("[M+HCO2]-",all_adducts),match("[M+HCOO]-",all_adducts),match("[M+Cl]-",all_adducts))
      ...
      if(grepl("+", paste(all_adducts,sep = "",collapse = "|")) && ("[M-H]-" %in% adduct_names || "[M-2H]-" %in% adduct_names || "[M+C2H3O2]-" %in% all_adducts || "[M+HCO2]-" %in% all_adducts || "[M+HCOO]-" %in% all_adducts ||"[M+Cl]-" %in% all_adducts)){
        negative_adduct_indices<-c(match("[M-H]-",adduct_names),match("[M-2H]-",adduct_names),match("[M+C2H3O2]-",adduct_names),match("[M+HCO2]-",adduct_names),match("[M+HCOO]-",all_adducts),match("[M+Cl]-",adduct_names))
        negative_adduct_indices<-negative_adduct_indices[!is.na(negative_adduct_indices)]
...
if(PosMode==TRUE) {
Directory_pos <- "D:/III_LM_FLOW_NEW/LipidMatch_Run/Output/PosIDed.csv"
...
if(NegMode==TRUE) {
Directory_neg <- "D:/III_LM_FLOW_NEW/LipidMatch_Run/Output/NegIDed.csv"

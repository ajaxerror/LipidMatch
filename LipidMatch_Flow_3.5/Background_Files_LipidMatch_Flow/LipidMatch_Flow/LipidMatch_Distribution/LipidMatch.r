    ...
    if (nrow(DataCombinedConfirmed) == 0) {
      ...
      if (length(ConfirmORcol) > 0) {
        ...
        for (i in ConfirmORcol) {
          columnToFill <- append(columnToFill, match(i, ConfirmORcol) +
            1 +
            length(ConfirmANDcol))
        }
...
remove_duplicates <- function(data, row_start, adduct_col, annotation_col, window, RT_col) {
  ...
  for (i in unique(top_ranked_lipid)) {
    ...
    if (length(rows) == 1) {
      ...
      if (grepl("+", paste(all_adducts, sep = "", collapse = "|")) && ("[M-H]-" %in% all_adducts ||
        "[M-2H]-" %in% all_adducts ||
        "[M+C2H3O2]-" %in% all_adducts ||
        "[M+HCO2]-" %in% all_adducts ||
        "[M+Cl]-" %in% all_adducts)) {
      ...
      if (grepl("+", paste(all_adducts, sep = "", collapse = "|")) && ("[M-H]-" %in% adduct_names ||
        "[M-2H]-" %in% adduct_names ||
        "[M+C2H3O2]-" %in% all_adducts ||
        "[M+HCO2]-" %in% all_adducts ||
        "[M+Cl]-" %in% all_adducts)) {
...
for (i in seq_len(lengthFoldersToRun)) {
  ...
  if (length(AIFMS2NEG_in) != 0 &&
    length(AIFMS1NEG_in) != 0 &&
    length(FeatureTable_NEG) == 1) { runNegAIF <- TRUE }
  if (length(AIFMS2POS_in) != 0 &&
    length(AIFMS1POS_in) != 0 &&
    length(FeatureTable_POS) == 1) { runPosAIF <- TRUE }
